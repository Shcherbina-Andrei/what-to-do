...
import { deleteTaskAction, editTaskAction } from '../../store/action';
...
function TaskCard({task}: TaskCardProps): JSX.Element {
  ...
  const editButtonHandle = () => {
    if (isEditing && (taskItem.description !== task.description)) {
  ...
  return (
    <article className="task-card">
      {isEditing ?
        <input className="task-card__input" type="text" value={taskItem.description} disabled={!isEditing} onChange={(evt) => setTaskItem({...taskItem, description: evt.target.value})}/>
        : <p className="task-card__description">{taskItem.description}</p>}
      <div className="task-card__controls">
        <button className="task-card__button task-card__button--edit" type="button" onClick={editButtonHandle}>
